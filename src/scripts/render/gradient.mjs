 ...
 * plus a blur. For a §3.4 spill that is the wrong representation, and three attempts to make a
 * falloff read as a gradient all failed against it rather than against their own mechanism:
 *
 * - Widening the blur spread each step further and added none. §6.4.2a's mechanism is one mesh's
 *   rim fading to reveal the mesh beneath, and beneath a stripe is only the next stripe.
 * - Sub-rings worked and were declined on looks: a linear ramp across a whole band leaves no
 *   plateau, so it reads as a smear rather than a light level.
 * - The blur's tap count was the visible banding. `PIXI.BlurFilter` spreads a fixed number of taps
 *   across its width, so a wide blur samples five points eleven pixels apart.
 *
 * A blur approximates a gradient with a filter; the rasteriser draws one for free. The level moves
 * from a shader uniform to a per-vertex attribute, and every triangle is interpolated barycentrically
 * at no extra pass. The consequence that matters is mesh count rather than smoothness — a whole
 * falloff becomes one mesh with one draw and no filter, against 69 meshes and 55 filtered render
 * passes before it.
 *
 * That header is the origin story; the file has since become the shared mesh pool for every pass
 * that layers over the ground partition. Four producers, one buffer layout, one shader:
 *
 ...
 *
 * Two payload shapes, differing in which half is expensive. Spill carries a distance per vertex and
 * is mapped here, its geometry costing a scene rebuild while its mapping is a slider. The rest carry
 * finished levels, their geometry being cheap enough to redo whenever they move.
 *
 * No mesh here is ever blurred. §6.4.4 blurs the whole container in one pass instead, and a filter
 * on a `MIN_COLOR` mesh is actively wrong besides: a blur fades the rim toward zero in premultiplied
 * colour, and zero in this channel is the brightest value there is.
 */
 ...
 *
 * @remarks
 * A thin wrapper since §6.4.3: the ladder becomes contiguous zones in scene pixels, handed to the
 * one profile every producer shares, so a spill band boundary fades over exactly the same distance
 * as a light's zone boundary or a room's edge.
 *
 * The wedge is the first zone and runs to `-Infinity`, which centres the transition on its own
...
function create(index) {
  ...
  // not. These buffers are uploaded by PIXI's dirty tracking, so it is a no-op.
 ...
 *
 * @remarks
 * No mesh here is blurred at all. Every wide-blur setting this file replaced existed to fake the
 * gradient the mesh now draws; what is left for a filter is the job it does on every other ground
 * mesh — taking the aliasing off the silhouette, here the wedge's flanks and wherever a wall cut it.
 * The falloff inside the mesh is no longer a boundary, and there is nothing for a blur to soften.
 */
function syncFilter(entry) {
  // No ramp mesh is ever blurred, since §6.4.3. Each already carries its own transition as a
  // per-vertex ramp, so a filter on top would be a second, differently-shaped softening of the same
  // edge — the piecemeal look this replaced. It is also unsafe on a blended mesh: a blur fades the
  // rim toward zero in premultiplied colour, and zero in this channel is the brightest value there
  // is, so `MIN` would ring every light in white.
 ...
 *
 * @remarks
 * Two payload shapes, differing in which half is expensive. §3.4's ramp carries a distance per
 * vertex and is mapped here, its geometry costing a scene rebuild while its mapping is a slider — so
 * a slider must not move a vertex. A §7.0 step 6 light carries finished levels, its geometry being
 * arithmetic redone whenever the light moves anyway, so keeping the mapping separate would buy
 * nothing.
 */
function uploadLevels(entry, ramp) {
  ...
  // over a gradient — see the note on the shader — so it is the ladder's midpoint, an approximation
  // documented rather than papered over.
  entry.mesh.shader.nominal = levels[Math.floor(levels.length / 2)] ?? levels[0];
  // Brighter ramps sort later and so win where two windows light the same floor — §3.4's `AT_LEAST`
  // rule as nearly as a draw order can express it.
...
function apply(entry, ramp) {
  ...
  // The blend mode is the architecture (§7.0 step 6). A ground ramp overwrites; a light blends
  // `MIN_COLOR`, so brightest wins per fragment; a clamp blends `MAX_COLOR`, so darkest does.
  // Together with the sort ladder in `render/darkness-shaders.mjs` that is the whole composition
  // rule, none of it involving cutting one region out of another.
...
/** Have the containers been replaced underneath? Same reasoning as `darkness-texture.stale`. */
function stale() {
  const first = pool[0];
 ...
 *
 * @remarks
 * `darkness-texture.paint` asks before skipping a spill cell: with no gradient mesh on the map the
 * flat bands are all there is, and skipping them would leave the spill unpainted.
 *
 * Narrowed to spill entries when §7.0 step 6 landed. A count of drawn meshes was the same question
 * while spill was the only producer and stopped being one the moment lights and clamps joined the
 * pool — and the failure would have been silent, a scene with torches and no windows reporting yes
 * and skipping bands nothing was drawing.
 */
 ...
 *
 * @remarks
 * Gated on `spill.version()` and the profile, so the common repaint — an observer moving — costs two
 * comparisons. That gate is the point of the design: the shadow clamp is applied by overpainting
 * rather than by cutting this geometry, so nothing here moves when a token does.
 *
...
export function sync(extra = [], { force = false } = {}) {
  ...
  // meshes. Passed in rather than pulled from here because they depend on the cells, and cells
  // belong to `render/paint.mjs`; reaching back for them would make this file depend on the pass
  // that calls it.
  const ramps = [...(spill.isEnabled() ? spill.ramps() : []), ...extra];

  // Only the cached half is gated. `spill.ramps()` is a pure cache read keyed on `spill.version()`
  // and can be skipped; `extra` is rebuilt by its caller every time, and that caller is itself gated
  // on the field and the point of view (`paint.repaint`). Gating on the spill version alone here
  ...
  lastStats = {
    ...
    // §7.0 step 6's cache. `builds` staying flat while a token is dragged is the reason a light's
    // grid is affordable to rebuild at all — see `light-ramps.cacheKey`.
 ...
 *
 * Wired to the plateau setting and the tier table, both of which change what a distance means and
 * neither of which moves a vertex.
 */
 ...
 *
 * @remarks
 * `ramps` above zero with `triangles` at zero is the state worth naming: the meshes exist and claim
 * ground they never paint, which is what a failed triangulation looks like from every other angle.
 * `sampled` is the texture read back at the cursor, the only number here proving a shader ran — see
 * `darkness-texture.meshAt` on why the two can disagree.
 */
export function stats() {
  const report = {
    enabled: spill.isEnabled(),
    ...(lastStats ?? { note: "no sync has run" }),
    // The sort ladder as it reached the meshes. Anything at or below a flat mesh's level would be
    // painted over by ordinary ground — see `render/darkness-shaders.mjs`.
